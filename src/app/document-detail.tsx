...
function statusLabel(status: string): { label: string; color: string; bg: string } {
  switch (status) {
    case 'approved': return { label: 'Đã duyệt', color: C.success, bg: C.successContainer };
    case 'pending': return { label: 'Chờ duyệt', color: '#7c4f00', bg: '#ffefc2' };
    case 'rejected': return { label: 'Bị từ chối', color: C.error, bg: C.errorContainer };
    case 'deleted': return { label: 'Đã xóa', color: C.outline, bg: C.surfaceContainer };
    default: return { label: status, color: C.outline, bg: C.surfaceContainer };
  }
}
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.topBarTitle} numberOfLines={1}>Chi tiết tài liệu</Text>
        {isOwner && (
          <View style={styles.topBarActions}>
            <Pressable style={styles.topBarBtn} onPress={handleEdit}>
              <SymbolView name="pencil" size={20} tintColor={C.primary} />
            </Pressable>
            <Pressable style={styles.topBarBtn} onPress={handleDelete} disabled={deleting}>
              {deleting
                ? <ActivityIndicator size="small" color={C.error} />
                : <SymbolView name="trash" size={20} tintColor={C.error} />
              }
            </Pressable>
          </View>
        )}
      ...
      >
        ...
        <View style={styles.previewWrap}>
          {doc.thumbnailUrl ? (
            ...
            <View style={styles.previewPlaceholder}>
              <SymbolView
                ...
                type="hierarchical"
              />
              <Text style={styles.previewFileType}>{doc.fileType.toUpperCase()}</Text>
            </View>
          )}
          ...
          <View style={[
            ...
            { backgroundColor: doc.visibility === 'public' ? C.primaryContainer : C.surfaceContainer }
          ]}>
            <SymbolView
        ...
        <View style={styles.section}>
          {doc.subject && (
            <Text style={styles.subjectCode}>{doc.subject.code} · {doc.subject.name}</Text>
          )}
          ...
          <View style={styles.statsRow}>
            <StatItem icon="arrow.down.to.line" value={formatCount(doc.downloadCount)} label="Lượt tải" />
            <View style={styles.statDivider} />
            <StatItem icon="eye" value={formatCount(doc.viewCount)} label="Lượt xem" />
            <View style={styles.statDivider} />
            <StatItem icon="doc.fill" value={`${doc.fileSizeMb} MB`} label="Dung lượng" />
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.uploaderRow}>
            <Image
              source={{ uri: doc.uploadedBy.avatarUrl ?? 'https://i.pravatar.cc/150?img=1' }}
          ...
          {comments.length === 0 ? (
            ...
            comments.map((c) => (
              <View key={c.id} style={styles.commentItem}>
                <Image
                  source={{ uri: c.user.avatarUrl ?? 'https://i.pravatar.cc/150?img=1' }}
            ...
            <View style={styles.commentInputWrap}>
              ...
              >
                {submitting
                  ...
                  : <SymbolView name="paperplane.fill" size={16} tintColor={C.onPrimary} />
                }
              </Pressable>
            </View>
          </View>
      ...
      <View style={[styles.bottomBar, { paddingBottom: insets.bottom || S.md }]}>
        <Pressable style={styles.previewBtn} onPress={() => Alert.alert('Preview', 'Tính năng xem trước PDF sẽ dùng PDF.js')}>
        ...
        <Pressable style={styles.downloadBtn} onPress={handleDownload}>
          <SymbolView name="arrow.down.to.line" size={18} tintColor={C.onPrimary} />
...
const styles = StyleSheet.create({
  ...
  loadingWrap: { flex: 1, justifyContent: 'center', alignItems: 'center', gap: S.md },
  ...
  previewPlaceholder: {
    width: '100%', height: 220, backgroundColor: C.surfaceLow,
    justifyContent: 'center', alignItems: 'center', gap: S.sm,
  },
  previewFileType: { fontSize: 13, fontWeight: '700', color: C.onSurfaceVariant },
  fileTypeBadge: {
  ...
  sectionTitle: { fontSize: 13, fontWeight: '700', color: C.onSurfaceVariant, textTransform: 'uppercase', letterSpacing: 0.5 },

  // Title
  subjectCode: { fontSize: 12, fontWeight: '600', color: C.primary, textTransform: 'uppercase' },
  title: { fontSize: 20, fontWeight: '700', color: C.onSurface, lineHeight: 28 },
  statusBadge: {
    alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 9999,
  },
  ...
  statsRow: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: C.surfaceLow, borderRadius: 8, padding: S.md,
    marginTop: 4,
  },
  ...
  tag: {
    ...
    backgroundColor: C.primaryContainer, borderRadius: 9999,
  },
  tagText: { fontSize: 12, color: C.primary, fontWeight: '500' },

  // Comments
  emptyComments: { fontSize: 13, color: C.onSurfaceVariant, fontStyle: 'italic', textAlign: 'center', paddingVertical: S.md },
  commentItem: { flexDirection: 'row', gap: S.sm, paddingVertical: S.sm, borderTopWidth: 1, borderTopColor: C.outlineVariant },
  ...
  commentInputRow: { flexDirection: 'row', gap: S.sm, alignItems: 'flex-end', marginTop: S.sm },
  commentInputWrap: {
    flex: 1, flexDirection: 'row', alignItems: 'flex-end',
    backgroundColor: C.surfaceLow, borderRadius: 12,
    borderWidth: 1, borderColor: C.outlineVariant,
    paddingHorizontal: S.md, paddingVertical: S.sm, gap: S.sm,
  },
  ...
  previewBtn: {
    ...
    gap: S.sm, paddingVertical: 14, borderRadius: 10,
